...
def generate_text_from_sound(audio_file):
    ...
    for result in response.results:
        text_output="Drawing now, please wait a minute."
        inputs = {'prompt': result.alternatives[0].transcript}
        print("request : ",inputs)
        image_output = version.predict("Draw me a cat")
        print(image_output)
        return image_output
...
def generate_image():
    print('drawing cat')
    url = replicate.run("tstramer/midjourney-diffusion:436b051ebd8f68d23e83d22de5e198e0995357afef113768c20f0b6fcef23c8b",input={"prompt": "a 19th century portrait of a wombat gentleman"})
    return url[0]
